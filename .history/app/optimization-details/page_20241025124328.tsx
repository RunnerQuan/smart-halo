...
export default function OptimizationDetails() {
  ...
  const highlightCode = (code: string) => {
    ...
    let highlightedCode = highlight(code, languages.javascript, 'javascript');
    
    // 然后处理 **code** 标记
    highlightedCode = highlightedCode.replace(
      /(<span[^>]*>.*?<\/span>)|(\*\*(.*?)\*\*)/gs,
      (match, highlightedPart, fullMatch, codeContent) => {
        if (highlightedPart) {
          // 如果是已经高亮的部分，直接返回
          return highlightedPart;
        } else {
          // 如果是 **code** 格式的内容，应用自定义高亮
          return `<span class="bg-yellow-300 text-black px-1 rounded">${codeContent}</span>`;
        }
      }
    );
