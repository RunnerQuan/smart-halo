...
export default function ContractOptimizationDetails() {
  ...
  const handleCopy = () => {
    const cleanCode = optimizedCode.replace(/\*\*(.*?)\*\*/g, '$1');
  ...
  const highlightCode = (code: string) => {
    // 使用唯一标识符替换 **code** 标记
    const placeholder = 'CUSTOM_HIGHLIGHT_PLACEHOLDER';
    let index = 0;
    const replacements: {[key: string]: string} = {};

    const codeWithPlaceholders = code.replace(/\*\*(.*?)\*\*/g, (match, content) => {
      const key = `${placeholder}_${index++}`;
      replacements[key] = content;
      return `<!--${key}-->`;
    });

    // 使用 Prism 进行 JavaScript 语法高亮
    let highlightedCode = highlight(codeWithPlaceholders, languages.javascript, 'javascript');

    // 恢复自定义高亮
    Object.entries(replacements).forEach(([key, content]) => {
      highlightedCode = highlightedCode.replace(
        `&lt;!--${key}--&gt;`,
        `<span class="bg-yellow-300 text-black px-1 rounded">${content}</span>`
      );
    });
