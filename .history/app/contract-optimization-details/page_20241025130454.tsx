...
export default function ContractOptimizationDetails() {
  ...
  const handleCopy = () => {
    ...
    tempDiv.innerHTML = optimizedCode.replace(/\*\*(.*?)\*\*/g, '$1');
  ...
  const highlightCode = (code: string) => {
    // 首先使用 Prism 进行 JavaScript 语法高亮
    let highlightedCode = highlight(code, languages.js, 'javascript');
    
    // 然后处理 **code** 标记，防止与 Prism 高亮冲突
    highlightedCode = highlightedCode.replace(
      /\*\*(.*?)\*\*/g, 
      (match, p1) => `<span class="bg-yellow-300 text-black px-1 rounded">${p1}</span>`
    );
