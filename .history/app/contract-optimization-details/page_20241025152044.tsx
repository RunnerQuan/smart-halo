...
export default function ContractOptimizationDetails() {
  ...
  const highlightSolidityCode = (code: string) => {
    // 首先使用 hljs 进行 Solidity 语法高亮
    let highlightedCode = hljs.highlight(code, { language: 'solidity' }).value;
    
    // 然后处理 **code** 标记，包括可能跨越多行的情况
    highlightedCode = highlightedCode.replace(
      /\*\*([\s\S]*?)\*\*/g,
      '<span style="background-color: #FFFF00; color: #000000; padding: 2px 4px; border-radius: 3px;">$1</span>'
    );
