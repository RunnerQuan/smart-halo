...
import solidity from 'react-syntax-highlighter/dist/esm/languages/hljs/solidity';
...
export default function ContractOptimizationDetails() {
  ...
  const highlightSolidityCode = (code: string) => {
    const highlightedCode = SyntaxHighlighter.highlight(code, {language: 'solidity', style: vs2015});
  ...
  const CustomSyntaxHighlighter = ({ code, language }) => {
    return (
      <pre className="syntax-highlighter" style={{...vs2015['pre[class*="language-"]'], background: 'transparent'}}>
        <code 
          dangerouslySetInnerHTML={{ __html: highlightSolidityCode(code) }}
          style={{...vs2015['code[class*="language-"]'], background: 'transparent'}}
        />
      </pre>
    );
  };
