...
export default function ContractOptimizationDetails() {
  ...
  const highlightSolidityCode = (code: string) => {
    // 首先处理 **code** 标记，包括函数体内的内容
    const preProcessedCode = code.replace(
      /(\*\*.*?\*\*)|(\bfunction\s+\w+\s*\([^)]*\)\s*{[\s\S]*?})/g,
      (match, p1, p2) => {
        if (p1) {
          // 处理 **code** 标记
          return `<span class="custom-highlight">${p1.slice(2, -2)}</span>`;
        } else if (p2) {
          // 处理函数体
          return p2.replace(/\*\*(.*?)\*\*/g, '<span class="custom-highlight">$1</span>');
        }
        return match;
      }
    );
