...
export default function ContractOptimization() {
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      // 解析响应体
      const responseData = await decompileResponse.json();  // 首次读取并解析 JSON
      ...
      const optimizeResponse = await fetch('http://172.18.197.84:5000/process_code', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  useEffect(() => {
    ...
    const checkTaskStatus = async () => {
      if (taskId) {
        try {
          const response = await fetch(`http://172.18.197.84:5000/task_status/${taskId}`);
