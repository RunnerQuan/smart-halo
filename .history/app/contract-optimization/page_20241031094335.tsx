...
export default function ContractOptimization() {
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      // 第一步：向本地的get.py发送请求获取反编译代码、源代码和字节码
      ...
      const { decompiled_code, source_code, bytecode } = responseData;
      ...
      sessionStorage.setItem('bytecode', bytecode);
    ...
    <main className="flex min-h-screen flex-col items-center justify-start p-8 bg-[#1A1A1A] text-white font-sans">
      ...
      <div className="mt-24 w-full max-w-4xl mx-auto flex flex-col items-center">
        ...
        >
          ...
          用户输入任意有效的智能合约地址，系统会从 Dedaub 平台上找到对应的合约的反编译代码和源代码。随后，SmartHalo 工具会对该反编译代码进行加强和优化，确保变量类型准确、合约属性恢复完整、函数边界清晰。
          </p>
          <p className="text-gray-300">
            优化后，您可以查看该反编译代码的优化详情，并对其进行漏洞检测。此外，我们还对优化后的代码进行高亮显示，便于用户阅读。
          </p>
        </motion.div>
      </div>
    </main>
