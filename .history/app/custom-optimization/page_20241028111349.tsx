...
export default function CustomOptimization() {
  ...
  const handleAnalyzeContract = useCallback(async () => {
    ...
    try {
      const response = await fetch('http://localhost:2525/extract_functions', {
        method: 'POST',
        headers: {
  ...
  const handleOptimize = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      console.log('Sending function name:', selectedFunction); // 添加日志查看发送的函数名
      ...
      const response = await fetch('http://localhost:2525/process_code', {
        ...
        body: JSON.stringify({ 
          code: contractCode,
          // 只有当 selectedFunction 为空字符串时才使用 'all_func'
          func_name: selectedFunction === '' ? 'all_func' : selectedFunction
        }),
      });
    ...
    const checkTaskStatus = async () => {
      if (taskId) {
        try {
          ...
          const response = await fetch(`http://localhost:2525${endpoint}`);
          const data = await response.json();
          ...
          <div className="flex justify-between items-center space-x-4">
            ...
            <Select
              ...
              onChange={(value: string) => {
                console.log('Selected function in dropdown:', value); // 添加日志
